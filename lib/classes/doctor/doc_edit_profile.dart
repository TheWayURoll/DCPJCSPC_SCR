...
class DocEditProfile extends StatefulWidget {
  final String? docId;
  const DocEditProfile({Key? key, this.docId}) : super(key: key);
...
class _DocEditProfileState extends State<DocEditProfile> {
  ...
  Future<void> _fetchDocData() async {
    ...
    if (query.docs.isNotEmpty) {
      ...
      _idCardController.text = data['docId'] ?? '';
      _nameController.text = data['docName'] ?? '';
      _phoneController.text = data['docDepart'] ?? '';
      _passwordController.text = data['docPassword'] ?? '';
    }
    setState(() => _loading = false);
  }

  Future<void> _updateDocData() async {
    if (!_formKey.currentState!.validate()) return;
    setState(() => _loading = true);
    final query = await FirebaseFirestore.instance
        .collection('doctor')
        .where('docId', isEqualTo: _idCardController.text)
        .limit(1)
        .get();
    if (query.docs.isNotEmpty) {
      await query.docs.first.reference.update({
        'docName': _nameController.text,
        'docDepart': _phoneController.text,
        'docPassword': _passwordController.text,
      });
    }
    setState(() => _loading = false);
              ...
              child: Padding(
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      _buildField(_idCardController, 'docId', readOnly: true),
                      const SizedBox(height: 24),
                      _buildField(_nameController, 'ชื่อ'),
                      const SizedBox(height: 24),
                      _buildField(_phoneController, 'แผนก'),
                      const SizedBox(height: 24),
                      _buildField(_passwordController, 'รหัสผ่าน', isPassword: true),
                      const SizedBox(height: 36),
                      _buildButton('ตกลง', onPressed: _updateDocData),
